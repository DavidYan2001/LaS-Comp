...
def sparse_cube2verts(coords, feats, training=True):
    """
    args:
    coords: 有效的voxel的序列坐标（0-255）： N*3
    feats: 有效的voxel的verts的预测内容：N*8*10

    return:
    new_coords: 有效的verts的序列坐标，无重复verts： n*3
    new_feats: 有效的verts位置的预测内容：n*10
    cube: 每个体素（cube）的 8 个角点在 verts_unique（也就是 new_coords）里的索引: N*8, verts_unique是总共有多少个verts
    """
    # print(coords.shape)
    # print(feats.shape)
